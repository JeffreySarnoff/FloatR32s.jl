...
import Base: convert, promote_rule, show, string,
             ...
             ==, !=, <, <=, >, >=, isless, isequal, 
             +, -, *, \, /, ^, fma, muladd,
             signbit, precision, significand, exponent, sign, 
             eps, inv, sqrt, cbrt, hypot, clamp, clamp!,
             min, max, minmax, frexp, ldexp, 
             abs, copysign, flipsign, zero, one, iszero, isone,
             isfinite, issubnormal, isinf, isnan, 
             floatmin, floatmax, maxintfloat, typemax, typemin,
             mod, rem, div, fld, cld, divrem, fldmod,
             floor, ceil, trunc, round,
             rand, randn, evalpoly,
             sum, prod, cumsum, cumprod, accumulate, 
             sum!, prod!, cumsum!, cumprod!, accumulate!

import Base.Math: abs2, acos, acosd, acosh, acot, acotd, acoth, 
                  acsc, acscd, acsch, asec, asecd, asech, 
                  asin, asind, asinh, atan, atand, atanh,
                  cos, cosc, cosd, cosh, cospi, cot, cotd, coth,
                  csc, cscd, csch, deg2rad, 
                  evalpoly, exp, exp10, exp2, expm1,
                  log, log10, log1p, log2, 
                  mod2pi, modf, rad2deg, rem2pi, 
                  sec, secd, sech,
                  sin, sinc, sincos, sincosd, sind, sinh, sinpi, 
                  tan, tand # sincospi
...
convert(::Type{Rational{T}}, x::Robust32) where {T} = convert(Rational{T}, Float32(x))
...
const Robust32_1 = Robust32(1.0)
const Robust32_2 = Robust32(2.0)

Robust32(x::Bool) = x ? Robust32_1 : Robust32_0
Bool(x::Robust32) = iszero(x) ? false : true
Base.hash(x::Robust32, h::UInt64) = Base.hash(Float32(x), h)

Base.decompose(x::Robust32) = Base.decompose(Float32(x))
...
isinf(x::Robust32) = isinf(Float64(x))
isnan(x::Robust32) = isnan(Float64(x))

signbit(x::Robust32) = signbit(Float32(x))
...
for F in (:(==), :(!=), :(<), :(<=), :(>), :(>=), :isless, :isequal)
  @eval begin
    $F(x::Robust32, y::Robust32) = $F(Float64(x), Float64(y))
    $F(x::Robust32, y::Float64) = $F(Float64(x), y)
    $F(x::Robust32, y::Float32) = $F(Float64(x), y)
    $F(x::Float64, y::Robust32) = $F(x, Float64(y))
    $F(x::Float32, y::Robust32) = $F(x, Float64(y))
    $F(x::AbstractFloat, y::Robust32) = $F(promote(x,y)...)
    $F(x::Robust32, y::AbstractFloat) = $F(promote(x,y)...)
    $F(x::Rational, y::Robust32) = $F(x, Float64(y))
    $F(x::Robust32, y::Rational) = $F(Float64(x), y)
    $F(x::Bool, y::Robust32) = $F(x, Float64(y))
    $F(x::Robust32, y::Bool) = $F(Float64(x), y)
  end  
end
...
for F in (:hypot, :clamp)
  @eval begin
    $F(x::Robust32, y::Robust32, z::Robust32) = Robust32($F(Float64(x), Float64(y), Float64(z)))
...
for F in (:abs2, :acos, :acosd, :acosh, :acot, :acotd, :acoth, 
          :acsc, :acscd, :acsch, :asec, :asecd, :asech, 
          :asin, :asind, :asinh, :atan, :atand, :atanh,
          :cos, :cosc, :cosd, :cosh, :cospi, :cot, :cotd, :coth,
          :csc, :cscd, :csch, :deg2rad,
          ...
          :log, :log10, :log1p, :log2, 
          :mod2pi, :rad2deg, :rem2pi, 
          :sec, :secd, :sech, :sin, :sinc, :sind, :sinh,
          :sinpi, :tan, :tand, :tanh)
    @eval $F(x::Robust32) = Robust32($F(Float64(x)))
end

atan(x::Robust32, y::Robust32) = Robust32(atan(Float64(x), Float64(y)))
...
for F in (:fma, :muladd)
  @eval begin
    $F(x::Robust32, y::Robust32, z::Robust32) =
        Robust32($F(Float64(x), Float64(y), Float64(z)))
    $F(x::Robust32, y::Robust32, z::T) where {T<:Real} = $F(promote(x,y,z)...)
    $F(x::T, y::Robust32, z::Robust32) where {T<:Real} = $F(promote(x,y,z)...)
    $F(x::Robust32, y::T, z::Robust32) where {T<:Real} = $F(promote(x,y,z)...)
    $F(x::Robust32, y::T, z::T) where {T<:Real} = $F(promote(x,y,z)...)
